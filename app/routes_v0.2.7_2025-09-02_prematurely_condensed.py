...
# NOTES
#
# A more extensive version of this file, including a number of potentially
# obsolete endpoint handlers, some obsolescent components, etc., has been saved
# as ./app/routes.py_v0.2.7_2025-09-02
# (Anything found subsequently to be useful can be recovered from that.)
#------------------------------------------------------------------------------
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if hub_mode == "omtrad":
            session["previous_page"] = "home_ahc"   # (This subsequently serves
            return redirect(url_for("home_ahc"))    # as a shift register).
        else:
            session["previous_page"] = "home"       # (This subsequently serves
            return redirect(url_for("home"))        # as a shift register).
    return render_template(
        "login.html",
        title = "Sign in",
        page = page,
...
def create_pairing(owner_fph = ""):
    ...
    if form.validate_on_submit():
        ...
        flash("New pairing created: " + ahid_hrns + " & " + currency_hrns)
        if hub_mode == "omtrad":
            return redirect("/home_ahc")
        else:
            return redirect("/home")
